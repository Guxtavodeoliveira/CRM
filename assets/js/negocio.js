...
function cabecalho(card){
  ...
  <div class="deal-head">
    ...
    <div class="deal-meta">
      ...
      <span class="sep">·</span>
      <span class="avatar">${esc(iniciais(dados.usuario))}</span>
      <span>${esc(dados.usuario)}</span>
      ${card.responsavelEmpresa ? `<span class="sep">·</span><span>Responsável: ${esc(card.responsavelEmpresa)}</span>` : ""}
    </div>
  </div>`;
}
...
function atividadeHtml(card, a){
  ...
  <div class="act ${a.concluido ? "done" : ""}">
    ...
    <div class="act-foot">
      ...
      <span class="g">Responsável <span class="avatar">${esc(iniciais(dados.usuario))}</span></span>
  ...
  <div class="panel">
    ...
    <div class="panel-body">
      <div class="kv">
        ...
        <span class="v">${editavel(card, "responsavelEmpresa", "text", "Adicionar")}</span>
      ...
      <div class="contact-scroll">
        <div class="contact-block">
          ...
          <div class="kv">
            ...
            ${card.inscricaoEstadual ? linha("Inscr. estadual", card.inscricaoEstadual, "") : ""}
            ${card.responsavelEmpresa ? linha("Responsável", card.responsavelEmpresa, "") : ""}
          </div>
        </div>

        ${(card.banco || card.agencia || card.contaCorrente || card.contaPoupanca || card.pix) ? `
        <div class="contact-block">
          <div class="block-label">Dados bancários</div>
          <div class="kv">
            ${card.banco ? linha("Banco", card.banco, "") : ""}
            ${card.agencia ? linha("Agência", card.agencia, "") : ""}
            ${card.contaCorrente ? linha("C/C", card.contaCorrente, "") : ""}
            ${card.contaPoupanca ? linha("Poupança", card.contaPoupanca, "") : ""}
            ${card.pix ? linha("Pix", card.pix, "") : ""}
          </div>
        </div>` : ""}
