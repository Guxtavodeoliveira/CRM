...
function linhasNegocios(base, opcoes){
  ...
  return cards.map(c => {
    ...
    return [
      ...
      base.usuario || "",                                         // Usuário responsável
...
const COLS_EMPRESAS = [
  "Nome fantasia","CNPJ","Razão social","Inscrição estadual","Responsável","Descrição",
  "Banco","Agência","Conta corrente","Conta poupança","Pix",
  "E-mail","WhatsApp","Telefone","Celular","Fax","Ramal","Website",
  "CEP","País","Estado","Cidade","Bairro","Rua","Número","Complemento",
  "Facebook","X (twitter)","LinkedIn","Skype","Instagram"
];
function linhasEmpresas(base){
  return base.cards.map(c => [
    c.nome, c.cnpj, c.razaoSocial, c.inscricaoEstadual, c.responsavelEmpresa, c.descricao,
    c.banco, c.agencia, c.contaCorrente, c.contaPoupanca, c.pix,
