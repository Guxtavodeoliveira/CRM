  ...
  const d = {
    ...
    cards: (negocios.data||[]).map(n => {
      ...
      return {
        ...
        responsavelEmpresa: c.responsavel_empresa || "",
        banco: c.banco || "", agencia: c.agencia || "",
        contaCorrente: c.conta_corrente || "", contaPoupanca: c.conta_poupanca || "",
        pix: c.pix || "",
...
/* =========================================================
   ...
   ========================================================= */
/* Enquanto o banco não tiver as colunas novas (arquivo
   banco/atualizar-campos.sql), grava sem elas para o sistema
   continuar funcionando em vez de dar erro. */
let clienteTemCamposNovos = true;
...
function paraCliente(c, ownerId){
  ...
  if(clienteTemCamposNovos){
    linha.inscricao_estadual  = c.inscricaoEstadual || "";
    linha.responsavel_empresa = c.responsavelEmpresa || "";
    linha.banco               = c.banco || "";
    linha.agencia             = c.agencia || "";
    linha.conta_corrente      = c.contaCorrente || "";
    linha.conta_poupanca      = c.contaPoupanca || "";
    linha.pix                 = c.pix || "";
  }
  return linha;
}

/** O banco reclama de coluna que não existe? É porque falta rodar o SQL. */
function faltaColunaNova(e){
  const msg = ((e && e.message) || "") + " " + ((e && e.details) || "");
  return /inscricao_estadual|responsavel_empresa|conta_corrente|conta_poupanca|'banco'|'agencia'|'pix'/i.test(msg)
      || (e && e.code === "PGRST204");
}

const CAMPOS_NOVOS_CLIENTE = ["inscricao_estadual","responsavel_empresa",
  "banco","agencia","conta_corrente","conta_poupanca","pix"];

let avisouCamposNovos = false;

/** Grava os clientes; se as colunas novas ainda não existem, grava sem elas. */
async function gravarClientes(linhas){
  let { error } = await sb.from("clientes").upsert(linhas);
  if(error && faltaColunaNova(error)){
    clienteTemCamposNovos = false;
    const simples = linhas.map(l => {
      const x = { ...l };
      CAMPOS_NOVOS_CLIENTE.forEach(k => delete x[k]);
      return x;
    });
    ({ error } = await sb.from("clientes").upsert(simples));
    if(!error && !avisouCamposNovos){
      avisouCamposNovos = true;
      toast("Cliente salvo, mas a inscrição estadual e os dados bancários não vão para a nuvem ainda: rode o arquivo banco/atualizar-campos.sql no Supabase.", "err");
    }
  }
  if(error) throw error;
}
...
async function sincronizar(){
  ...
  try{
    ...
    if(dCli.gravar.length) await gravarClientes(dCli.gravar);
