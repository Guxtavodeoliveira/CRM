...
> **Atualizando para a versão com dados bancários?** Antes de subir os arquivos,
> rode o `banco/atualizar-campos.sql` no Supabase. O passo a passo está em
> `LEIA-PRIMEIRO-ATUALIZACAO.md`, aqui nesta mesma pasta.
...
Ela contém o `schema.sql`, o `migrar.js` e o `atualizar-campos.sql`. O
`atualizar-campos.sql` é o único que você precisa rodar agora (veja
`LEIA-PRIMEIRO-ATUALIZACAO.md`); os outros dois são de instalação inicial. Subir junto não atrapalha nada — são arquivos parados, não
